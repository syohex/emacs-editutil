...
(defun editutil-format-buffer ()
  ...
  (let ((args (cl-case major-mode
                ((c-mode c++-mode) '("clang-format" "-i" :file))
                (go-ts-mode '("go" "fmt"))
                ((rust-mode rust-ts-mode) '("cargo" "fmt"))
                (tuareg-mode (if (editutil--dune-project-p)
                                 '("dune" "fmt")
                               '("ocamlformat" "-i" "--enable-outside-detected-project" :file)))
                (otherwise (user-error "unsupport formatting for %s" major-mode)))))
    (pcase args
      (`(,cmd . ,options)
       (let ((options (mapcar (lambda (arg) (if (eq arg :file) (buffer-file-name) arg)) options)))
...
(defun editutil-default-setup ()
  ...
  (keymap-global-set "C-k" #'editutil-kill-line)
  (keymap-global-set "C-y" #'editutil-yank)
  (keymap-global-set "C-w" #'editutil-kill-region)

  (keymap-global-set "M-F" #'editutil-next-word)
  (keymap-global-set "M-B" #'editutil-previous-word)
  (keymap-global-set "M-e" #'editutil-forward-char)
  (keymap-global-set "M-E" #'editutil-backward-char)
  (keymap-global-set "M-h" #'editutil-mark-to-char)
  (keymap-global-set "M-o" #'editutil-edit-next-line)
  (keymap-global-set "M-O" #'editutil-edit-previous-line)
  (keymap-global-set "M-w" #'editutil-kill-ring-save)
  (keymap-global-set "M-q" #'editutil-zap-to-char)
  (keymap-global-set "M-Q" #'editutil-zap-to-char-back)
  (keymap-global-set "M-d" #'editutil-delete-word)
  ...
  (keymap-global-set "C-M-n" #'editutil-forward-list)
  ...
  (keymap-global-set "C-x l" #'editutil-copy-line)
  ...
  (keymap-global-set "M-g M-r" #'recompile)

  ;; flymake
  (keymap-global-set "M-g M-n" #'editutil-next-error)
  (keymap-global-set "M-g M-p" #'editutil-previous-error)
  (keymap-global-set "C-h l" #'flymake-show-buffer-diagnostics)
  (keymap-global-set "C-h i" #'editutil-show-current-line-diagnostic)
  ...
  (keymap-global-set "C-h V" #'find-variable)

  ;; windmove
  (keymap-global-set "M-g h" #'windmove-left)
  (keymap-global-set "M-g j" #'windmove-down)
  (keymap-global-set "M-g k" #'windmove-up)
  (keymap-global-set "M-g l" #'windmove-right)

  ;; search-map
  (keymap-set search-map "SPC" #'point-to-register)
  (keymap-set search-map "j" #'jump-to-register)
  (keymap-set search-map "i" #'insert-register)
  (keymap-set search-map "c" #'copy-to-register)
  (keymap-set search-map "m" #'bookmark-set)
  (keymap-set search-map "b" #'bookmark-jump)
  ...
  (with-eval-after-load 'paredit
    (keymap-set paredit-mode-map "M-q" #'editutil-zap-to-char)
    (keymap-set paredit-mode-map "C-c l" #'editutil-toggle-let)
    (keymap-set paredit-mode-map "DEL" #'editutil-paredit-backward-delete))
